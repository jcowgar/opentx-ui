...
function menu.handle_edit_key_event_button(mnu, values, keyEvent)
	local m = mnu[mnu.CurrentField]
end
...
function menu.handle_edit_key_event(mnu, values, keyEvent)
	...
	elseif mType == menu.TYPE_BUTTON then
		return menu.handle_edit_key_event_button(mnu, values, keyEvent)
	end
end
...
function menu.execute(mnu, values, keyEvent)
	...
	if mnu.IsEditing then
		...
		menu.handle_nav_key_event(mnu, values, keyEvent)
	end
	...
	for mIndex = 1, #mnu do
		...
		lcd.drawText(m[LAYOUT_X], m[LAYOUT_Y], mLabel)
